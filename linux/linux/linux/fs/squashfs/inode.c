			...
			long long *next_index)
{
	...
	if (compressed) {
		...
		start = k;
		for (; k < b; k++) {
			memcpy(c_buffer + bytes, bh[k]->b_data + offset,
			       avail_bytes);
			bytes += avail_bytes;
			offset = 0;
			brelse(bh[k]);
		...
		len = msblk->read_size;
		len = LzmaUncompress (buffer, len, c_buffer, bytes);
...
static struct super_block *squashfs_read_super(struct super_block *s, void *data, int silent)
{
	kdev_t dev = s->s_dev;
	struct squashfs_sb_info *msblk = &s->u.squashfs_sb;
	struct squashfs_super_block *sblk = &msblk->sblk;
				...
				const char *name, int size)
{
	...
	int i, length = 0;
	struct squashfs_dir_index *index;
	char str[sizeof(struct squashfs_dir_index) +
...
static int squashfs_readdir(struct file *file, void *dirent, filldir_t filldir)
{
	...
	char str[sizeof(struct squashfs_dir_entry) + SQUASHFS_NAME_LEN + 1];
	struct squashfs_dir_entry *dire = (struct squashfs_dir_entry *)str;
...
MODULE_DESCRIPTION("squashfs 3.2-r, a compressed read-only filesystem");
