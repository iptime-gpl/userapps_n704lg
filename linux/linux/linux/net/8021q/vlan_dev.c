...
int vlan_dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (veth->h_vlan_proto != __constant_htons(ETH_P_8021Q)) {
		...
		/* If the frame is priority tagged (vlan id 0) then as
		 * per 802.1q-1998 (Sec 5.1 Static conformance requirements)
		 * we update the tag header with vlan id of the outgoing
		 * interface.
		 */
