 ...
 *
 * Version:	$Id: arp.c,v 1.2 2009/11/05 01:14:16 bcmnew Exp $
 *
 ...
 */
	...
	/* Special case: IPv4 duplicate address detection packet (RFC2131) */
        ...
        {
                struct net_device *sdev = ip_dev_find(sip);
                if(sdev != dev)
                {
                }
                else
...
#ifdef CONFIG_IPCONFLICT_DETECT
static int arp_ip_conflict(char *buffer, char **start, off_t offset, int length)
{
        int len=0;
        off_t pos=0;
        int size;

        size = sprintf(buffer,"%08x %02x:%02x:%02x:%02x:%02x:%02x %s",
                        conflict_ip,
...
#ifdef CONFIG_IPCONFLICT_DETECT
        proc_net_create ("ipconflict", 0, arp_ip_conflict);
#endif
