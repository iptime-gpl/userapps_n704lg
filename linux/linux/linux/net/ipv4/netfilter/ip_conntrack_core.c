...
#ifdef HNDCTF
extern int ip_conntrack_ipct_delete(struct ip_conntrack *ct, int ct_timeout);
#endif /* HNDCTF */
...
#ifdef HNDCTF
	ip_conntrack_ipct_delete(h->ctrack, 0);
#endif /* HNDCTF */
...
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP
        if(!check_conntrack_ratio_per_ip(tuple, &passme))
                return ERR_PTR(-ENOMEM);
#endif
...
ip_ct_port_restricted_cone_nat_tuple(struct ip_conntrack_tuple *tuple, struct ip_conntrack *conntrack)
{
        ...
        target_mask.src.u.all = 0xffff;
        target_mask.dst.ip = 0;
        target_mask.dst.u.all = 0xffff;
        ...
        if (tuple_hash)
        {
                ...
                printk("=> ORG tuple %u %u.%u.%u.%u:%hu -> %u.%u.%u.%u:%hu\n",  \
                        ...
                        ntohs(conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all),
                ...
                printk("=> PORT Restricted Cone Old tuple %p: %u %u.%u.%u.%u:%hu -> %u.%u.%u.%u:%hu\n",       \
                       ...
                       NIPQUAD(tuple->src.ip), ntohs(tuple->src.u.all),         \
                       NIPQUAD(tuple->dst.ip), ntohs(tuple->dst.u.all));

#endif
                tuple->src.u.all =
                        tuple_hash->ctrack->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.all;
...
int __init ip_conntrack_init(void)
{
	 ...
	 * machine has 256 buckets.  >= 1GB machines have 8192 buckets. */
 	if (hashsize) {
		...
		if (ip_conntrack_htable_size <= 64)
			sysctl_ip_conntrack_tcp_timeouts[TCP_CONNTRACK_ESTABLISHED] = 5 HOURS; /* 5 Hours */
	}
#ifndef CONFIG_IP_CONNTRACK_MAX_DEFINE
