...
/* scchoi added for special charater can't pass argv in iptables 0x9 0xA 0xB 0xC 0xD */
int memcmp_sp( char *t, char *n, int len )
{
	int i;
	for( i = 0; i < len ; i++ )
	{
...
char *search_linear (char *needle, char *haystack, int needle_len, int haystack_len) 
{
	...
	while ( t++ < k ) {
		...
		if ( memcmp_sp(t, needle, needle_len) == 0 ) 
			/* needle found */
			return t;
	}
	/* needle not found */
