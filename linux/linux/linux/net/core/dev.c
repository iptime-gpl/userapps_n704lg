...
//            && (skb->nfmark || !strncmp(dev->name, "wl", 2)) // mssid interface
//#endif
	) {
...
#ifdef CONFIG_QOS_INTERFACE_MARK
        if (skb->dev->qdisc && !strncmp(skb->dev->name, "wl", 2))
        {
                int index, ifidx;
                sscanf(skb->dev->name,"wl%d.%d", &ifidx, &index);
                skb->nfmark = (INTERFACE_MARK_SUBID_BASE*(ifidx+1))+index;
        }
#endif
