...
/* #ifdef	BCMINTERNAL	-- Commented out to get an external nightly build with it,
 *				   will put it back tomorrow.
 */
	{
		unsigned long *nvptr;
		unsigned long start, end;
		unsigned int sdram_init = 0, cfrac;

		nvptr = (unsigned long *)((unsigned int)KSEG1ADDR(0x1fc00000 /* SI_FLASH1 */) + 1024);
		if (*nvptr == 0x48534C46 /* NVRAM_MAGIC */) {
			sdram_init = nvptr[2] >> 16;	/* Get sdram_init */
			if ((sdram_init == 0x0419) || (sdram_init == 0xffff))
				sdram_init = 0;
		}

		/* Icache fraction in bits 13:12 of sdram_init */
		cfrac = 1 << ((sdram_init >> 12) & 3);
		if (cfrac != 1) {
			end = icache_size;
			icache_size /= cfrac;
			start = icache_size;
			printk("I Cache Frac is %d, using only %dkb of the cache\n",
			       cfrac, icache_size >> 10);

			printk("Locking I cache indices 0x%lx - 0x%lx\n", start, end);
			write_c0_idatalo(0);	/* cleanliness */
			write_c0_idatahi(0);
			write_c0_itaghi(0);
			while (start < end) {
				cache_unroll(start, Index_Invalidate_I);
				write_c0_itaglo(0x70000000 | start | 0xa0);
				cache_unroll(start, Index_Store_Tag_I);
				start += ic_lsize;
			}
		}

		/* Dcache fraction in bits 15:14 of sdram_init */
		cfrac = 1 << ((sdram_init >> 14) & 3);
		if (cfrac != 1) {
			end = dcache_size;
			dcache_size /= cfrac;
			start = dcache_size;
			printk("D Cache Frac is %d, using only %dkb of the cache\n",
			       cfrac, dcache_size >> 10);

			printk("Locking D cache indices 0x%lx - 0x%lx\n", start, end);
			write_c0_ddatalo(0);	/* cleanliness */
			write_c0_dtaghi(0);
			while (start < end) {
				cache_unroll(start, Index_Writeback_Inv_D);
				write_c0_dtaglo(0x70000000 | start | 0xa0);
