...
/*	case CPU_20KC:*/
	...
	case CPU_74K:
		if(cpu_wait_enable) {
			cpu_wait = r4k_wait;
			if ((mips_cpu.processor_id & 0xff) > 0x40) 
...
__init void cpu_probe(void)
{
	...
	case PRID_COMP_MIPS:
		...
		case PRID_IMP_74K:
			 ...
			 */
			mips_cpu.cputype = CPU_74K;
			mips_cpu.options &= ~MIPS_CPU_DIVEC;
			mips_cpu.isa_level = MIPS_CPU_ISA_M32;
#ifdef SW_HAZARD_BARRIERS
			/* Clear the IHB bit in config7 */
...
__init void cpu_report(void)
{
	printk("CPU ProcId is: 0x%08x, options: 0x%08x\n",
	       mips_cpu.processor_id, mips_cpu.options);
	if (mips_cpu.options & MIPS_CPU_FPU)
