      ...
      struct ipt_entry_target **target)
{
	...
	case '1':
		if (string_to_number(optarg, 0, 0xffffffff, 
			...
			exit_error(PARAMETER_PROBLEM, "Bad RTMARK value `%s'", optarg);
		if (*flags)
			exit_error(PARAMETER_PROBLEM,
			           "RTMARK target: Can't specify --set-mark twice");
		*flags = 1;
		break;
...
final_check(unsigned int flags)
{
	if (!flags)
		exit_error(PARAMETER_PROBLEM,
		           "RTMARK target: Parameter --set-mark is required");
}
