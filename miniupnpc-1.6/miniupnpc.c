...
// Enable only MINISSDP is used. Currently, we only have DLNA application for MINISSDP  
//in rtlac tree, we found serious bug in A2004NS-R. Maybe in unix.c Kernel ( connect system call will make system halt
#ifdef USE_DLNA
	...
	if(ipv6) {
		...
		memset(&p->sin6_addr,0x0,sizeof(p->sin6_addr));	
	} else {
