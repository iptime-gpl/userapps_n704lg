/* $Id: iptcrdr.c,v 1.21 2015/08/28 09:34:40 hymt7621 Exp $ */
/* MiniUPnP project
				   ...
				   const char * desc)
{
	...
	if(r >= 0)
	{
		add_redirect_desc(eport, proto, desc);
                ...
                if ((!strncmp(desc, "ipDISK_Server_FTP", strlen("ipDISK_Server_FTP")) || strstr(desc, "ipdisk_ftp"))
		    && iport!= 21)
                {
                        int idx;
		...
		while (strcmp(nat_chain_name[++n], ""))
		{
...
#endif
			{
				if(proto==e->ip.proto)
				{
					...
					{
					char desc[20];
					get_redirect_desc(eport, proto, desc, 20);
					if (!strncmp(desc, "ipDISK_Server_FTP", strlen("ipDISK_Server_FTP")) && iport != 21) 
